/** BOT Testnet-only MultiSend deployer. Requires the approved signer and explicit --broadcast. */
...
if (config.chainId !== 968 || config.deploymentApproved !== true) throw new Error("DEPLOYMENT_NOT_APPROVED");
...
if (account.address.toLowerCase() !== owner.toLowerCase()) throw new Error("SIGNER_IS_NOT_APPROVED_OWNER");
...
const hash = await walletClient.deployContract({ abi: artifact.abi, bytecode: artifact.bytecode.object, args: [owner, feeRecipient] });
...
if (live.owner.toLowerCase() !== owner.toLowerCase() || live.feeRecipient.toLowerCase() !== feeRecipient.toLowerCase() || live.feeBps !== 1 || live.maxRecipients !== 100 || live.configNonce !== 0 || live.paused) {
  throw new Error("POST_DEPLOY_STATE_MISMATCH");
}

const manifest = {
  release: "MultiSend V1",
  network: "bot-testnet",
  chainId: 968,
  address,
  ...
  runtimeBytecodeSha256: createHash("sha256").update(code).digest("hex"),
...
console.log(JSON.stringify({ verdict: "DEPLOYED_CHAIN_STATE_PASS_EXPLORER_VERIFICATION_PENDING", chainId: 968, address, deployTxHash: hash, live }, null, 2));