/**
 * FlowBridge MultiSend V1 — BscScan (BNB Testnet, chain 97) source verification.
 *
...
const api = `${config.explorerApiUrl}?chainid=97`;
...
console.log(JSON.stringify({ verdict: verified ? "BNB TESTNET SOURCE VERIFICATION PASS" : "BNB TESTNET SOURCE VERIFICATION FAIL", record }, null, 2));
if (verified) {
  manifest.explorerVerified = true;
  manifest.explorerVerification = record;
  writeFileSync(
    join(REPO, "contracts/deployments/multisend-bnb-testnet.json"),
