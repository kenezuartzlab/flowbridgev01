/**
 * BNB Testnet ERC-20 rehearsal + live negative acceptance for MultiSend V1.
 *
 ...
 *
 * Mainnet (chain 56) with an owner-approved production ERC-20 only. No test
 * token is ever deployed or used here. Requires the approved signer and
 * --broadcast.
 */
...
const config = JSON.parse(readFileSync(join(REPO, "contracts/config/multisend-bnb-mainnet.json"), "utf8"));
const artifact = JSON.parse(
  readFileSync(join(REPO, (process.env["MULTISEND_ARTIFACT"] ?? "contracts/production/multisend-v1/candidate-rc2/artifacts/FlowBridgeMultiSend.json")), "utf8"),
);
const manifestPath = join(REPO, (process.env["MULTISEND_MANIFEST"] ?? "contracts/deployments/multisend-bnb-mainnet.json"));
...
if (config.chainId !== 56) throw new Error("MAINNET_CHAIN_ONLY");
...
/* ----------------------------------------------- approved production token -- */
/**
 * The token must be named explicitly by the FlowBridge owner. Nothing is
 * deployed on mainnet and no test asset may be used.
 */
const token = getAddress(
  process.argv.find((a) => /^0x[0-9a-fA-F]{40}$/.test(a)) ??
    (() => {
      throw new Error("APPROVED_PRODUCTION_TOKEN_REQUIRED");
    })(),
) as Address;
const TEST_ONLY_TOKENS = ["0xa861152ca3676bccf7b5fdafb9eb6a57b9d32d0e"];
if (TEST_ONLY_TOKENS.includes(token.toLowerCase())) throw new Error("TEST_TOKEN_REJECTED_ON_MAINNET");
...
const tokenRead = <T>(functionName: string, args: unknown[] = []) =>
  publicClient.readContract({ address: token, abi: tokenAbi, functionName, args } as never) as Promise<T>;
const DECIMALS = Number(await tokenRead<number>("decimals"));
const TOKEN_SYMBOL = await tokenRead<string>("symbol");
const tokenDeployTx = null;
...
const groups = [
  {
    ...
    amounts: [unit("1"), unit("2"), unit("0.55")],
  },
  { mode: "consolidate", recipients: [recipient("destination")], amounts: [unit("3")] },
  { mode: "advanced", recipients: [recipient("a1"), recipient("a2")], amounts: [unit("1.5"), unit("2.5")] },
  { mode: "duplicate-recipients", recipients: [recipient("dup"), recipient("dup")], amounts: [unit("0.1"), unit("0.2")] },
];
...
for (const group of groups) {
  ...
  positives.push({
    ...
    feeAsset: `${TOKEN_SYMBOL} (same token as the transfer)`,
...
await expectRejection(`over-${maxRecipients}-recipients`, "TooManyRecipients", () =>
  simulate({
    ...
    amounts: Array.from({ length: maxRecipients + 1 }, () => unit("0.0001")),
  }),
);
await expectRejection("insufficient-token-balance", "transferFrom balance check", () =>
  simulate({ recipients: [recipient("n5")], amounts: [unit("900000000")] }),
);
...
const report = {
  gate: "MULTISEND_V1_BNB_MAINNET_ERC20_AND_ADVERSARIAL_REHEARSAL",
...
const outputPath = join(REPO, "contracts/production/multisend-v1/release/bnb-mainnet-rehearsal-erc20.json");
