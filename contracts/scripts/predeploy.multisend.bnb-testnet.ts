/**
 * FlowBridge MultiSend V1 — BNB Smart Chain Testnet (97) predeployment gate.
 *
 * Read-only. No signing, no broadcast. Every check must pass before the
 * BNB Testnet creation transaction may be prepared. The build line is the
...
add("CHAIN_ID_97_CONFIGURED", config.chainId === 97, `config chain ${config.chainId}`);
...
add("BNB_TESTNET_SLOT_UNDEPLOYED", !existsSync(manifestPath), "no existing BNB testnet manifest");
...
add("RPC_CHAIN_IS_97", rpcChainId === 97, `RPC reports ${rpcChainId}`);

if (rpcChainId === 97 && signer) {
  ...
  JSON.stringify(
    {
      verdict: failures.length
        ? "FLOWBRIDGE MULTISEND BNB TESTNET PREDEPLOYMENT BLOCKED"
        : "FLOWBRIDGE MULTISEND BNB TESTNET PREDEPLOYMENT PASS",
      chainId: rpcChainId,
      owner,
      feeRecipient,
      deployer,
