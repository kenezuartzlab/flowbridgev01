// V30.2B R4 — FlowStakingRewardTreasury reproducibility build.
// Exact frozen matrix read from contracts/production/V30_2A_REDEPLOY_PREFLIGHT.json
// replacements[R4]: solc 0.8.24+commit.e11b9ed9, optimizer enabled runs 200,
...
const input = {
  language: 'Solidity',
  sources: {
    'FlowStakingRewardTreasury.sol': {
      content: fs.readFileSync(path.join(SRC, 'FlowStakingRewardTreasury.sol'), 'utf8'),
    },
  },
  settings: {
...
function compile() {
  ...
  return {
    a: out.contracts['FlowStakingRewardTreasury.sol'].FlowStakingRewardTreasury,
...
const res = {
  ...
  sourceSha256: h(fs.readFileSync(path.join(SRC, 'FlowStakingRewardTreasury.sol'))),
