...
/* ------------------------------------------------------------ test token -- */
const tokenManifestPath = join(REPO, "contracts/deployments/multisend-testtoken-bnb-testnet.json");
const DECIMALS = 18;
...
if (existsSync(tokenManifestPath)) {
  ...
  writeFileSync(
    ...
    JSON.stringify(
      {
        purpose: "MultiSend V1 BNB Testnet ERC-20 rehearsal asset only — never mainnet, never product code",
        network: "bnb-testnet",
        chainId: 97,
        address: token,
        symbol: "MSTT",
...
const report = {
  gate: "MULTISEND_V1_BNB_TESTNET_ERC20_AND_ADVERSARIAL_REHEARSAL",
...
const outputPath = join(REPO, "contracts/production/multisend-v1/release/bnb-testnet-rehearsal-erc20.json");
