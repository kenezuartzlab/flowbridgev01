/**
 * FlowBridge V9 — mobile bottom navigation (max 5 destinations), driven by the
 * single canonical nav model so it can never drift from the desktop nav.
 * Trade is the emphasized first-class centre destination. Hidden on desktop,
 * where PrimaryNav takes over. Purely presentational — it never touches
 * swap/bridge execution state.
 */
...
export function BottomNav() {
  ...
  return (
    <>
      ...
      <div aria-hidden className="h-[66px] sm:h-[70px] md:hidden" />
      <nav
              ...
              <li key={dest.id} className="min-w-0 flex-1">
                ...
                >
                  ...
                  <span
                    ...
                    className={
                      center
                        ? "-mt-4 grid h-11 w-11 shrink-0 place-items-center rounded-2xl border border-primary/45 bg-primary/20 text-primary shadow-[0_0_0_4px_color-mix(in_srgb,var(--fb-primary)_10%,transparent),0_8px_20px_-8px_color-mix(in_srgb,var(--fb-primary)_70%,transparent)] transition-transform group-hover:scale-105 motion-reduce:transition-none"
