/**
 * FlowBridge MultiSend V1 — BOT Mainnet (677) deployer.
 *
 * Deploys the frozen RC2 build line (unchanged source, self-contained
 * Standard-JSON bundle) with the approved production owner and fee recipient.
...
if (config.chainId !== 677 || config.deploymentApproved !== true)
...
if (
  ...
  artifact.abiSha256 !== testnet.abiSha256 ||
...
const manifest = {
  release: "MultiSend V1 (RC2 build line) — BOT Mainnet",
  network: "bot-mainnet",
  chainId: 677,
  address,
  ...
  acceptedTestnetRelease: testnet.address,
...
console.log(
  JSON.stringify(
    {
      verdict: "BOT_MAINNET_DEPLOYED_CHAIN_STATE_PASS",
      chainId: 677,
      address,
      deployTxHash: hash,
