...
/**
 * V9.2 — the Campaign Studio workspace, now owned by the secure Sets console.
 * `embedded` drops the standalone page chrome so it composes inside Sets.
 */
export function CampaignStudioWorkspace({ embedded = false }: { embedded?: boolean }) {
  const StudioShell = embedded ? EmbeddedStudioShell : FullStudioShell;
...
function EmbeddedStudioShell({ children }: { children: React.ReactNode }) {
  return <div className="space-y-3">{children}</div>;
}

function FullStudioShell({ children }: { children: React.ReactNode }) {
  return (
