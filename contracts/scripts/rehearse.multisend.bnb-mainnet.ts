/**
 * BNB Mainnet native rehearsal for MultiSend V1.
 *
...
if (account.address.toLowerCase() !== String(config.approvedDeployer).toLowerCase()) throw new Error("SIGNER_IS_NOT_APPROVED_WALLET");

const rpc = process.env["BNB_MAINNET_RPC_URL"] ?? config.rpcUrl;
const chain = defineChain({ id: 56, name: "BNB Smart Chain Mainnet", nativeCurrency: { name: "BNB", symbol: "BNB", decimals: 18 }, rpcUrls: { default: { http: [rpc] } } });
...
const groups = [
  { mode: "distribute", recipients: [recipient("d1"), recipient("d2"), recipient("d3")], amounts: [parseEther("0.00001"), parseEther("0.00002"), parseEther("0.000005")] },
  { mode: "consolidate", recipients: [recipient("destination")], amounts: [parseEther("0.00003")] },
  { mode: "advanced", recipients: [recipient("a1"), recipient("a2")], amounts: [parseEther("0.000015"), parseEther("0.000025")] },
  { mode: "duplicate-recipients", recipients: [recipient("dup"), recipient("dup")], amounts: [parseEther("0.00001"), parseEther("0.00002")] },
];
...
const report = {
  gate: "MULTISEND_V1_BNB_MAINNET_NATIVE_REHEARSAL",
  verdict: "PASS",
  chainId: 56,
  contract: address,
...
const outputPath = join(REPO, "contracts/production/multisend-v1/release/bnb-mainnet-rehearsal-native.json");
