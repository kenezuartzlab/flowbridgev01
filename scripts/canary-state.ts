...
import { mapAdapterRequestState } from '../src/lib/bridge/adapterStatus';
...
for(let i=0;i<5;i++){
 ...
 console.log('state',Number(s),JSON.stringify(mapAdapterRequestState(Number(s))),'canClaimRefund',claim);
 if(Number(s)>=2)break;
