      ...
      >
        ...
        <div className="flex justify-between items-center pb-3 border-b border-white/5 font-mono">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-sm font-black text-white uppercase tracking-wider">Cloud Ledger</h3>
              {email && (
                <p className="text-[11px] text-[#00D7B2] font-semibold truncate max-w-[200px]" title={email}>
                  Logged in: {email}
                </p>
              )}
            </div>
        ...
        <div className="flex-1 overflow-y-auto py-3 pr-1 space-y-2.5 mt-2 scrollbar-thin">
          {transactions.length === 0 ? (
            <div className="text-center py-10 bg-[#010C1B]/54 border border-white/5 rounded-2xl">
              <p className="text-sm font-semibold text-[#F0F7F3]">No transactions logged yet.</p>
              <p className="text-[12px] text-[#C5C1B9] mt-2.5 max-w-[200px] mx-auto leading-relaxed">
            ...
            transactions.map((tx: any) => (
              <div 
                key={tx.id} 
                className="p-3 bg-[#010C1B]/40 border border-white/5 hover:bg-[#010C1B]/80 transition-colors rounded-xl flex items-center justify-between text-sm"
              >
                <div className="flex flex-col min-w-0 pr-2">
                  <div className="flex items-center gap-1.5 font-mono">
                    ...
                    <span className="font-bold text-white truncate text-[12px] uppercase tracking-wide">
                      {tx.direction.replace('_', ' → ')}
                    </span>
                  </div>
                  <div className="text-[12px] text-[#C5C1B9] mt-2 flex items-center gap-1 font-mono">
                  ...
                  {tx.txHash && (
                    <a 
                      ...
                      target="_blank" 
                      rel="noreferrer"
                      className="text-[#32FF8B] hover:underline font-mono text-[11px] mt-1.5 truncate max-w-[150px] inline-flex items-center gap-0.5"
                    >
                ...
                <div className="text-right shrink-0 flex flex-col items-end gap-1.5 font-mono">
                  <span className="text-[11px] text-[#C5C1B9] block">
                    {new Date(tx.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}
                  </span>
                  <div className="flex items-center gap-1 bg-[#32FF8B]/10 border border-[#32FF8B]/25 text-[#32FF8B] px-1.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider">
